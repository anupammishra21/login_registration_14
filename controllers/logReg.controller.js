...
const fs = require("fs");
const os = require('os');
const { has, object } = require("underscore");
...
async showForgetPasswordForm(req, res) {
  // let update_password = await logReg.findOne({ _id: req.params.id });
    
    try {
      
      res.render("forgetPassword", {
        title: "show forgetPassword form",
        error: req.flash("error"),
  ...
  async PasswordUpdate(req,res){
    try{
      ...
      const new_password =  `p${Math.round(Math.random() * 10000)}` 
      console.log(new_password);

      const hash_password = bcrypt.hashSync(new_password , bcrypt.genSaltSync(10));

    // console.log("new password",hash_password);

    let updated_obj = {       
      password:hash_password,
  }

    let update_Password = await logReg.findByIdAndUpdate(is_email_exists._id, updated_obj );


    // console.log(update_Password);


    if (!_.isEmpty(update_Password)) {
      const dateTimeObject = new Date();
      await mailer.sendMail(process.env.EMAIL, is_email_exists.email, 'Password Changed Successfully!!!', `Hi ${is_email_exists.name} Your password changed successfully.<br> New Password : ${new_password} <br> Date : ${dateTimeObject.toDateString()} <br> Time : ${dateTimeObject.toTimeString()} <br> Operating System : ${os.type()} <br> Platform Name : ${os.platform()} `);
      req.flash('success','Password succesfully updated');
      res.redirect('/show-login')                
  } else {
      req.flash('error','There was some error. Please try again');
      ...
      // await mailer.sendMail(process.env.EMAIL, is_email_exists.email, 'password changed sucessfully', `Hi ${is_email_exists.email} .<br> password is :  `);
